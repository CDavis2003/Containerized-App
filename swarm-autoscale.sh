...
#sudo docker-compose build -t webapp:1.0 .
...
# CONFIGURATION
#####################################
SERVICE_NAME="web"        # name of your swarm service
MIN_REPLICAS=3                   # never scale below this
MAX_REPLICAS=6                  # never scale above this
UPPER_THRESHOLD=70.0             # if avg CPU > this, scale up
LOWER_THRESHOLD=30.0             # if avg CPU < this, scale down
...
while true; do
  ...
  if [[ $NEW -ne $CURRENT ]]; then
    ...
    echo "✔️  No scaling action needed"
  fi
